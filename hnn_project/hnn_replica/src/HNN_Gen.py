...
class HNN:
    ...
    def __init__(self, C, eps):
        ...
        # Default is 2**6; allow override via env var HNN_N (e.g., 32 for 2**5)
        self.N = int(os.getenv('HNN_N', 2**6))
        # Definition of the admissible number of memories
        #self.P = int(np.floor(self.N/10))
        self.P = int(os.getenv('NUM_MEMORIES', '10'))
        #self.P = int(np.floor(self.N/(10*np.log(self.N))))
    ...
    def net(self):
        if self.Co == 'O':
            ...
            C[1,1] = -1
            C0 = np.copy(C)
            for j in range(int(np.log2(self.N/2))):
                C = np.kron(C,C0)
            C = C[:,1:]
        elif self.Co == 'R':
